...
    It creates a file named wayfire-<display_name>-<pid>.socket inside that directory (or inside /tmp/ if XDG_RUNTIME_DIR is missing).
...
When an external client script wants to talk to Wayfire, it must open this exact socket file path and initiate a connection. Wayfire listens for these new connections by plugging the socket directly into the main Wayland event loop using wl_event_loop_add_fd. This means Wayfire checks for IPC messages at the same time it checks for mouse movements or window redraw requests. 3. The Transport Protocol: Length-Prefixed JSON
...
    Capabilities: \* The client provides the target view's ID.
...
Plugins can emit custom events. If an event name ends in a specific format or is triggered via custom_event_signal_t, the IPC will broadcast it. This allows third-party Wayfire plugins to talk to third-party IPC clients without modifying the core IPC source code. 9. Transaction Blocking and Window Interception
...
    An IPC client subscribes to the "view-pre-map" event.

    An application tries to open a window.

    Wayfire creates a transaction for this new window but pauses it. It creates an ipc_delay_object_t and injects it into the transaction manager.

    Wayfire sends the view-pre-map JSON event over the socket to the client, saying "Window ID 45 is trying to open."

    Wayfire entirely stops processing that window. The window will remain invisible.

    The IPC client (a Python script) analyzes the window. It sees that the window belongs to "Spotify". The script decides it wants Spotify to be exactly 800x600 pixels and moved to Workspace 3.

    The script sends "window-rules/configure-view" to apply these changes.

    Finally, the script sends the "window-rules/unblock-map" method back to Wayfire.

    Wayfire receives the unblock command, removes the ipc_delay_object_t from the transaction manager, and resumes drawing.

    The window finally appears on the screen, exactly where the script wanted it, without ever "flickering" in the wrong location first.
...
    Preparation: The IPC Plugin loads and creates /tmp/wayfire-...socket.

    Connection: The client opens the socket and connects. Wayfire registers the file descriptor to wl_event_loop.

    Transmission: The client crafts a JSON object { "method": "wayfire/get-config-option", "data": { "option": "core/vwidth" } }.

    Framing: The client calculates the byte length of the string (e.g., 85 bytes). It sends \[85 as 4-bytes\]\[The JSON String\].

    Reception: The Wayfire event loop detects activity on the file descriptor. It reads the 4 bytes, allocates an 85-byte vector, and reads the payload.

    Parsing: Wayfire parses the string into a wf::json_t object.

    Routing: Wayfire looks at the "method" string and queries the method_repository.

    Execution: The repository routes the call to the get_config_option C++ lambda function.

    Interfacing: The C++ function queries the Wayfire config-manager, retrieves the integer value for core/vwidth, and creates a new wf::json_t response.

    Return: The C++ function returns the JSON. The transport layer calculates the response length, prepends the 4-byte header, and writes it back to the socket file descriptor.

    Completion: The client script reads the 4-byte header, reads the JSON, and successfully knows the virtual width of the compositor.
